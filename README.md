...
app opens. The trigger word it recorded is offered when you open the folder, so there is nothing
to retype — and tick its **Prepare for Idiot LoRa Builder** box on export and the set arrives
**already triaged**, because it writes the same `.lora-studio/ratings.json` used here, marking
blurry, badly-exposed and near-duplicate shots as *needs edit*.
